...
RUN echo "root:p455w0rd" | /usr/sbin/chpasswd
RUN sed -i "s/PermitEmptyPasswords no/PermitEmptyPasswords yes/" /etc/ssh/sshd_config

# add user account with sudo access
RUN useradd -s /bin/bash -m bob
RUN echo "bob:p455w0rd" | /usr/sbin/chpasswd
RUN echo "bob ALL=NOPASSWD: ALL" >> /etc/sudoers
...
ENV MEMSQL_LICENSE c954423e81f0449ab3aec1c5924d414e
...
RUN cd /tmp \
 ...
 && ./memsql-ops-$OPS_VERSION/install.sh \
    --no-cluster --no-start --management-user bob \
 && rm -rf /tmp/*
